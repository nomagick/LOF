...
#include <sys/socket.h>
#include <netdb.h>
#define LOF_MAX_RECV_BUF_SIZE 61140
const char* global_ssi_uri = "https://uid.fetion.com.cn/ssiportal/SSIAppSignInV4.aspx";

int LOF_CONNECTION_FetionConnection_keep_alive(int socketfd)
{
	int keepAlive = 1;
#ifdef TCP_KEEPIDEL
	int keepIdle = 10;
	...
	if(setsockopt(socketfd , SOL_SOCKET , SO_KEEPALIVE 
		...
		LOF_debug_error("set SO_KEEPALIVE failed\n");
		return -1;
	}
	...
	if(setsockopt(socketfd , SOL_TCP , TCP_KEEPIDLE 
		...
		LOF_debug_error("set TCP_KEEPIDEL failed\n");
		return -1;
	}
#endif
	...
	if(setsockopt(socketfd , SOL_TCP , TCP_KEEPINTVL
		...
		LOF_debug_error("set TCP_KEEPINTVL failed\n");
		return -1;
	}
#endif
	...
	if(setsockopt(socketfd , SOL_TCP , TCP_KEEPCNT
		...
		LOF_debug_error("set TCP_KEEPCNT failed\n");
		return -1;
	}
#endif
	return 1;
...
LOF_CONNECTION_FetionConnectionType* LOF_CONNECTION_FetionConnection_new(void)
{
	int sfd;
...
//		return NULL;
	...
	LOF_CONNECTION_FetionConnectionType* conn = (LOF_CONNECTION_FetionConnectionType*)malloc(sizeof(LOF_CONNECTION_FetionConnectionType));
	if(conn == NULL){
		return NULL;
	}
	memset(conn , 0 , sizeof(LOF_CONNECTION_FetionConnectionType));
...
LOF_CONNECTION_FetionConnectionType* LOF_CONNECTION_FetionConnection_new_with_port(const int port)
{
	struct sockaddr_in addr;
	
	int sfd = socket(AF_INET , SOCK_STREAM , 0);
	if(LOF_CONNECTION_FetionConnection_keep_alive(sfd) == -1){
		return NULL;
	}
	...
	if(ret == -1){
		close(sfd);
		LOF_debug_error("Failed to bind");
		return NULL;
	}
	
	LOF_CONNECTION_FetionConnectionType* conn = NULL;
	conn = (LOF_CONNECTION_FetionConnectionType*)malloc(sizeof(LOF_CONNECTION_FetionConnectionType));
	if(conn == NULL){
		close(sfd);
		return NULL;
	}
...
LOF_CONNECTION_FetionConnectionType* LOF_CONNECTION_FetionConnection_new_with_ip_and_port(const char* ipaddress , const int port)
{
	struct sockaddr_in addr;
	int sfd = socket(AF_INET , SOCK_STREAM , 0);
	if(LOF_CONNECTION_FetionConnection_keep_alive(sfd) == -1){
...
int LOF_CONNECTION_FetionConnection_connect(LOF_CONNECTION_FetionConnectionType *connection, const char *ipaddress, const int port)
{
	struct sockaddr_in addr;
	int n;
	addr.sin_family = AF_INET;
...
char* LOF_TOOL_get_ip_by_name(const char* hostname)
{
	struct hostent *hst;
	char *name , *pos;
	int len;
...
int LOF_CONNECTION_FetionConnection_select_read(LOF_CONNECTION_FetionConnectionType* connection)
{
	fd_set fs ;
	struct timeval tv ;
	tv.tv_sec = 1;
...
int LOF_CONNECTION_FetionConnection_recv(LOF_CONNECTION_FetionConnectionType* connection , void* buf , int len)
{
	return recv(connection->socketfd , buf , len , 0);
}

int LOF_CONNECTION_FetionConnection_recv_dont_wait(LOF_CONNECTION_FetionConnectionType* connection , void* buf , int len)
{
...
int LOF_CONNECTION_FetionConnection_getname(LOF_CONNECTION_FetionConnectionType* connection , char **ip , int *port)
{
	struct sockaddr_in addr;
	int ret;
	socklen_t len = 0;
...
void LOF_CONNECTION_FetionConnection_close(LOF_CONNECTION_FetionConnectionType *connection)
{
	close(connection->socketfd);
	LOF_debug_info("Close connection with %s:%d",
...
char* LOF_CONNECTION_FetionConnection_ssl_connection_get(LOF_CONNECTION_FetionConnectionType* conn , const char* buf)
{
	char* ret;
...
char *LOF_CONNECTION_http_connection_get_response(LOF_CONNECTION_FetionConnectionType *conn)
{
	char buf[1024 * 4];
	char *res = 0, *res1 = 0, *pos = 0;
	int c = 0, res_len = 0;

	while((c = LOF_CONNECTION_FetionConnection_recv(
...
int LOF_TOOL_http_connection_get_body_length(const char* http)
{
	char *pos , length[10];
	int len;
...
int LOF_TOOL_http_connection_get_head_length(const char* http)
{
...
void LOF_CONNECTION_FetionConnection_free(LOF_CONNECTION_FetionConnectionType* conn)
{
...
char* LOF_TOOL_http_connection_encode_url(const char* url)
{
	char pos;
	char* res;
	char tmp[2];
...
char *LOF_TOOL_get_local_ip(void){
#if 0
    struct ifreq req;
    int sock;
	char *ip = NULL;
	...
	if ( ioctl(sock, SIOCGIFADDR, &req) < 0 ) {
		LOF_debug_error("Get local ip address failed");
		return NULL;
	}
...
char *LOF_TOOL_hexip_to_dotip(const char *ip){
